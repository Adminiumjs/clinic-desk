...
`docker compose up` gives you a working product: `clinic-db` creates the
...
By default these talk to the `clinic-db` container through
...
```bash
DATABASE_URL=postgres://user:pass@host:5432/rowan npm run demo:status
```
